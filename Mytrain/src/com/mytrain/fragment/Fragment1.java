...
public class Fragment1 extends Fragment {
	...
	private Time sendTime = new Time();
	...
	public void initFragment1() {
		...
		sendTime.set(calendar.get(Calendar.DAY_OF_MONTH),
	...
	class myListener2 implements View.OnClickListener {
		...
		public void onClick(View arg0) {
			...
			if (timeMillis <= 1000) {
				sendTime.setToNow();
				sendTime.set(0, 0, 0, sendTime.monthDay, sendTime.month,
						sendTime.year);
				timeMillis = sendTime.toMillis(true);
			}
			long timem2s = timeMillis / 1000;
			int timesec = (int) timem2s;
	...
	public void initListView() {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View view,
				...
				terminal_station = historyList.get(position)
						.getTerminal_station();
		...
		tv_clear.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				hd.deleteHistory();
				refreshListView();
				Toast.makeText(getActivity(), "历史记录已清除", Toast.LENGTH_LONG)
						.show();
			}
		});
	}
	...
	public void setHistoryList() {
		...
		for (int i = 0; i < size; i++) {
			...
			History history = historyList.get(num++);
			String origin = history.getOrigin_station();
			String terminal = history.getTerminal_station();
			Boolean flag = false;
			for (int j = i - 1; j >= 0; j--) {
				History old = historyList.get(j);
				if (old.getOrigin_station().equals(origin)
						&& old.getTerminal_station().equals(terminal)) {
					flag = true;
					break;
				}
			}
		...
		simpleadapter = new SimpleAdapter(getActivity(), historylists,
				R.layout.item_history, new String[] { "origin", "terminal" },
				new int[] { R.id.tv_originStation, R.id.tv_terminalStation });
	...
	public void setDialog() {
		dialog = new DatePickerDialog(getActivity(),
				...
				}, sendTime.year, sendTime.month, sendTime.monthDay);
	}
}
